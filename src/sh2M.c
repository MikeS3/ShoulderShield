...
static void sensorhubInputNormalHdlr(void *cookie, uint8_t *payload, uint16_t len, uint32_t timestamp) {
    ...
    while (cursor < len) {
        ...
        sh2_SensorEvent_t event;
        event.timestamp_uS = timestamp; // Or better: calculate timestamp properly
        event.reportId = reportId;
        event.len = reportLen;
        memcpy(event.report, payload + cursor, reportLen);

        // Store latest event and mark as available
        pSh2->latestEvent = event;
        pSh2->hasEvent = true;

        if (pSh2->sensorCallback) {
            pSh2->sensorCallback(pSh2->sensorCookie, &event);
        }
...
int sh2_getSensorEventInstance(sh2_Instance_t *instance, sh2_SensorEvent_t *event) {
    if (!instance || !event) return SH2_ERR_BAD_PARAM;

    *event = instance->latestEvent;
    instance->hasEvent = false;  // mark event as read
    return SH2_OK;
}
